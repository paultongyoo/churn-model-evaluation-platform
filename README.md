...
| Area | Features |
| ---- | -------- |
| 🧠 **Model Development** | <ul><li>[Jupyter Notebook ↗](https://jupyter.org/) provides Data Scientist-friendly environment for continued exploratory data analysis and data preparation.</li><li>Training logic extracted to separate module to ensure reuse of exact same logic in model inference pipeline.</li><li>Efficient hyperparameter and data preparation experimentation enabled using [Optuna ↗](https://optuna.org/) bayesian trial optimization and [MLflow ↗](https://mlflow.org/) experiment tracking.</li><li>Developer-friendly codebase implemented with best practices in mind, including [GitHub Actions-based CI/CD ↗](https://github.com/features/actions), pre-commit linting, and unit/integration tests.</li></ul> |
...
| 🚀 **Model Deployment** | <ul><li>MLflow Model Registry packages model type, parameters, inputs, and dependencies for every experiment run, ensuring model can be reliably deployed to new environments.</li><li>Alias-based model promotion allows model development to continue independently of model deployment, with deployment achieved by simply aliasing the new model.</li><li>Scalable and observable model inferencing using [Prefect ↗](https://www.prefect.io/) orchestration deployed on AWS Elastic Container Service (ECS).</li></ul> |
| 📈 **Model Monitoring** | <ul><li>[Evidently.ai ↗](https://www.evidentlyai.com/) integration enables automated data drift and model performance reporting for each load of new churn data.</li><li>Pre-built [Grafana ↗](https://grafana.com/grafana/) dashboard enables monitoring of data drift and performance over time, helping distinguish anomalies from meaningful signals that indicate the need for model development.</li></ul> |
